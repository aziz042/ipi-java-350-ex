...
public class Employe {
    ...
    public Integer getNbRtt(LocalDate d) {
        ...
        switch (LocalDate.of(d.getYear(), 1, 1).getDayOfWeek()) {
            ...
            case FRIDAY:
                if (d.isLeapYear()) joursOuvres = joursOuvres + 2;
        ...
        int monInt = (int) Entreprise.joursFeries(d)
                .stream()
                .filter(localDate -> localDate.getDayOfWeek().getValue() <= DayOfWeek.FRIDAY.getValue())
                .count();

        return (int) Math.ceil(
                (i1 - Entreprise.NB_JOURS_MAX_FORFAIT - joursOuvres - Entreprise.NB_CONGES_BASE - monInt)
                        * tempsPartiel
        );
    }
